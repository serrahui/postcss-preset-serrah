...
const baseInput = `
.button {
  background: light-dark(var(--serrah-color-red-5), var(--serrah-color-blue-9));
}
`;
...
@media screen and (min-width: 400px) {
  .button {
    background: light-dark(var(--serrah-color-green-5), var(--serrah-color-yellow-9));
  }
}
`;
...
.button-light-dark:hover {
  color: light-dark(var(--serrah-color-red-5), var(--serrah-color-blue-9));
}
`;
